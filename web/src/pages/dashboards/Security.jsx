...
export default function SecurityDashboard() {
  ...
  const handleStartQuiz = (assignment, quiz) => {
    ...
    navigate('/security/quiz', { state: { assignment, quiz } });
  };
    ...
    <>
      ...
      <div className="container my-4" style={{maxWidth:1200}}>
        <div className="text-center mb-4">
          ...
          <h1 className="display-4 fw-bold text-gradient mb-3">Security Team Dashboard</h1>
            ...
            <div className="col-12 col-md-6 col-lg-4">
              <div className="card h-100 border-0 glass-card hover-lift">
                <div className="card-body">
                  ...
                  <Link to="/security/games" className="btn btn-outline-primary btn-sm">
                    Play Games
                  </Link>
                </div>
              </div>
            ...
            <div className="col-12 col-md-6 col-lg-4">
              <div className="card h-100 border-0 glass-card hover-lift">
                <div className="card-body">
                  ...
                  <Link to="/security/progress" className="btn btn-outline-primary btn-sm">
                    View Progress
                  </Link>
                </div>
              </div>
                ...
                <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                  {quizAssignments.map((assignment) => {
                    ...
                    if (!quiz && !assignment.quizTitle) {
                      fetchQuizIfNeeded(assignment.quizId).then(() => {});
                    }

                    if (!quiz) {
                      quiz = { id: assignment.quizId, title: assignment.quizTitle || 'Quiz Assignment', description: assignment.quizDescription || 'Complete this training quiz', questions: assignment.questions || [], roles: assignment.roles || ['security'], topic: assignment.topic || assignment.quizTopic || null, isFallback: true };
                    }
        ...
        <Page title="Recent Policies" icon="bi-shield-check-fill">
          <div className="mb-3"><small className="text-muted">Policies found: {policies.length} | Acknowledged: {policyAcks.length}</small></div>
          {policies.length === 0 ? (
            <div className="text-center py-4"><i className="bi bi-file-text text-muted" style={{ fontSize: '2rem' }}></i><p className="text-muted mt-2 mb-0">No recent policies for your role</p></div>
          ) : (
            <>
              <h6 className="mb-2">Unread Policies</h6>
              <div className="row g-3 mb-4">{policies.filter(p => !policyAcks.some(a => a.policyId === p.id)).map(policy => <div key={policy.id} className="col-md-6 col-lg-4"><PolicyCard policy={policy} onAcknowledge={acknowledgePolicy} /></div>)}</div>
              <h6 className="mb-2">Acknowledged Policies</h6>
              <div className="row g-3">{policies.filter(p => policyAcks.some(a => a.policyId === p.id)).map(policy => <div key={policy.id} className="col-md-6 col-lg-4"><PolicyCard policy={policy} onAcknowledge={null} /></div>)}</div>
            </>
          )}
        </Page>
            ...
            <div className="col-md-6">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-body p-4">
                  ...
                  <p className="text-muted small mb-3">
                    As a security team member, you have access to view policies for all roles and ensure compliance across the organization.
                  </p>
                  <Link to="/policies" className="btn btn-outline-primary">
