      ...
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-start mb-2">
          <div className="d-flex align-items-center gap-2">
            ...
            <h6 className="fw-semibold mb-0">{fact.title || "Update"}</h6>
          </div>
          <div className="text-muted small">{formatDate(when)}</div>
        </div>
        <p className="mb-2">{message}</p>
        <div className="d-flex justify-content-end">
          <button className="btn btn-sm btn-outline-primary" onClick={() => onMarkRead && onMarkRead(fact.id)}>Mark Read</button>
        </div>
      </div>
    </div>
  );
...
export default function MarketingDashboard() {
  ...
  const {
    ...
    acknowledgePolicy,
    getUserDisplayName
  } = useRoleDashboardData('marketing');
  ...
  const handleStartQuiz = (assignment, quiz) => {
    if (!quiz) { alert('Quiz data not available.'); return; }
    navigate('/marketing/quiz', { state: { assignment, quiz } });
  };
    ...
    <>
      ...
      <div className="container my-4" style={{maxWidth:1200}}>
        <div className="text-center mb-4">
          ...
          <h1 className="display-4 fw-bold text-gradient mb-3">Marketing Team Dashboard</h1>
        ...
        <div className="alert alert-info border-0 shadow-sm mb-4 d-flex align-items-center" role="alert">
          ...
          <div><small className="fw-semibold">Session Info:</small><small className="text-muted ms-2">Auto-logout after {sessionTimeout} minutes of inactivity</small></div>
        </div>
          ...
          <div className="row g-3">
            <div className="col-12 col-md-6 col-lg-4">
              <div className="card h-100 border-0 glass-card hover-lift">
                <div className="card-body">
                  <div className="d-flex align-items-center mb-2"><div className="icon-wrapper me-2"><i className="bi bi-controller"></i></div><div className="fw-bold">Security Awareness Games</div></div>
                  <div className="text-body-secondary small mb-3">Interactive mini-games to test your security knowledge.</div>
                  <Link to="/marketing/games" className="btn btn-outline-primary btn-sm">Play Games</Link>
                </div>
              </div>
            </div>
            <div className="col-12 col-md-6 col-lg-4">
              <div className="card h-100 border-0 glass-card hover-lift">
                <div className="card-body">
                  ...
                  <Link to="/report-incident" className="btn btn-outline-danger btn-sm">Report Incident</Link>
                </div>
              </div>
            </div>
            <div className="col-12 col-md-6 col-lg-4">
              <div className="card h-100 border-0 glass-card hover-lift"><div className="card-body"><div className="d-flex align-items-center mb-2"><div className="icon-wrapper me-2"><i className="bi bi-graph-up"></i></div><div className="fw-bold">Training Progress</div></div><div className="text-body-secondary small mb-3">View your completed training modules and scores.</div><Link to="/marketing/progress" className="btn btn-outline-primary btn-sm">View Progress</Link></div></div>
            </div>
          </div>
        </Page>
        ...
        <div className="row g-4">
          <div className="col-lg-6">
            <Page title="Security Updates" icon="bi-bell-fill">
              {facts.length === 0 ? (
                <div className="text-center py-4"><i className="bi bi-lightbulb text-muted" style={{ fontSize: '2rem' }}></i><p className="text-muted mt-2 mb-0">No recent security updates</p></div>
              ) : (
                <div style={{ maxHeight: '400px', overflowY: 'auto' }}>{facts.map(f => {
                  const isRead = factReads.some(r => r.factId === f.id);
                  return (
                    <div key={f.id} className={isRead ? 'opacity-75' : ''}>
                      <NotificationCard fact={f} onMarkRead={isRead ? null : markFactRead} />
                    </div>
                  );
                })}</div>
              )}
          ...
          <div className="col-lg-6">
            <Page title="Assigned Quizzes" icon="bi-patch-question-fill">
              {quizAssignments.length === 0 ? (
                <div className="text-center py-4"><i className="bi bi-check-circle text-success" style={{ fontSize: '2rem' }}></i><p className="text-muted mt-2 mb-0">No pending quizzes</p></div>
              ) : (
                <div style={{ maxHeight: '400px', overflowY: 'auto' }}>{quizAssignments.map(assignment => {
                  let quiz = quizzes.find(q => q.id === assignment.quizId);
                  if (!quiz && !assignment.quizTitle) fetchQuizIfNeeded(assignment.quizId).then(() => {});
                  if (!quiz) quiz = { id: assignment.quizId, title: assignment.quizTitle || 'Quiz Assignment', description: assignment.quizDescription || 'Complete this training quiz', questions: assignment.questions || [], roles: assignment.roles || ['marketing'], topic: assignment.topic || assignment.quizTopic || null, isFallback: true };
                  return <QuizCard key={assignment.id} assignment={assignment} quiz={quiz} onStartQuiz={handleStartQuiz} />
                })}</div>
              )}
            </Page>
          </div>
